...
Gabarito (mapa de cobertura do Anexo B): deve recuperar **PROC-042v2-B**; a versão antiga (PROC-042-B) pode aparecer com relevância menor — risco de contradição (1.0 vs 1.1).

Server `retrieval-corpus` (filesystem) com escopo restrito a `data/retrieval-corpus/`. Rodada de 2026-06-10.

---

## Server: `retrieval-corpus`

Comando: `npx -y @modelcontextprotocol/server-filesystem ./data/retrieval-corpus` (cwd: `/Users/andre.vieira/Downloads/novatech-assistant`)
...
```
Allowed directories:
/Users/andre.vieira/Downloads/novatech-assistant/data/retrieval-corpus
```

### Listar o corpus disponível
...
```json
{
  "name": "list_directory",
  "arguments": {
    "path": "/Users/andre.vieira/Downloads/novatech-assistant/data/retrieval-corpus"
  }
}
```
...
### Ler o corpus de chunks para seleção por relevância (corpus completo — a seleção do top-k é feita pelo agente)
...
```json
{
  "name": "read_text_file",
  "arguments": {
    "path": "/Users/andre.vieira/Downloads/novatech-assistant/data/retrieval-corpus/chunks-novatech.md"
  }
}
```
...
## Seleção do agente (simulação do top-k do retrieval)

Para a pergunta **"Qual o multiplicador de frete para o Sudeste?"**, os chunks selecionados por relevância foram:

1. **PROC-042v2-B** (selecionado como fonte da resposta): *"Multiplicadores regionais atualizados (novembro/2023): Sul 1.3, **Sudeste 1.1**, Centro-Oeste 1.4, Nordeste 1.5, Norte 1.8."*
2. **PROC-042-B** (recuperado, mas descartado por vigência — ADR-0003): *"Multiplicadores regionais (PROC-042 v1): ... Sudeste 1.0 ..."* — versão antiga, mantida no corpus apenas marcada como obsoleta.
3. **FAQ-08** (relevância menor, confirma a regra de desambiguação): *"existem duas versões da PROC-042... Na dúvida, use a mais recente (v2)"*.

**Resultado vs. gabarito (Anexo B, mapa de cobertura):** ✅ match — o gabarito pede PROC-042v2-B como chunk obrigatório e prevê PROC-042-B como ruído de contradição, tratado aqui pela regra de vigência.

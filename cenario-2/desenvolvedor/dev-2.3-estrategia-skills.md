...
- **7 anti-padrões** observados em código gerado por IA — incluindo um real desta fase: `zod` em `devDependencies` no scaffold, que quebraria o deploy (detectado e corrigido no exercício 2.2).

As regras da skill já estão exercitadas pelo código da T01 (`src/shared/types.ts`, `src/functions/query/validator.ts`, `src/functions/query/handler.ts`) — a skill não é aspiracional, descreve o padrão que o repositório de fato segue.
